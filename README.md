# API - Marketplace de Livros


* ###  [Kotlin - Docs ](https://kotlinlang.org/docs/home.html)
* [Referencia para Query com Spring JPA](https://www.baeldung.com/spring-jpa-like-queries)
* [Spring Boot Gradle Plugin Reference Guide](https://docs.spring.io/spring-boot/docs/2.6.1/gradle-plugin/reference/html/)
* [Flyway Documentation - Spring](https://flywaydb.org/documentation/usage/plugins/springboot.html)
* [Flyway Documentation - Migrations](https://flywaydb.org/documentation/concepts/migrations#sql-based-migrations#syntax)
* [Flyway Documentation - MySQL](https://flywaydb.org/documentation/database/mysql)
* [MVN - Repository](https://mvnrepository.com/)

